...
def preprocess_sentences(input_json_file,save_json_file):
    ...
    for doc in tqdm(data):
        ...
        ner_candidates = doc.get('predicted_ner', [[]])
        if not ner_candidates or not ner_candidates[0]:
            ner_candidates = doc.get('ner', [[]])

        rel_candidates = doc.get('predicted_relations', [[]])
        if not rel_candidates or not rel_candidates[0]:
            rel_candidates = doc.get('relations', [[]])
        
        predicted_entities = {}
        for entity_info in ner_candidates[0]:
            start, end, entity_type = entity_info
            entity_text = ' '.join(doc['sentences'][0][start:end + 1])
            predicted_entities[entity_text] = entity_type
        
        predicted_relations = []
        for relation_info in rel_candidates[0]:
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='Reverse structure PURE prediction output')
    parser.add_argument('--input_json_file', type=str, default='./ent_pre_your_test_file.json')
    parser.add_argument('--save_json_file', type=str, default='../../data/your_test_file.json')
