...
def batched_index_select(target, indices):
    ...
    expanded_indices = indices.expand(-1, -1, target.size(-1))
    return torch.gather(target, 1, expanded_indices)
    ...
    def __init__(self, input_dim, num_layers, hidden_dims, activations, dropout):
        ...
        for layer_idx in range(num_layers):
            ...
            if activations is not None:
                layers.append(activations if layer_idx == num_layers - 1 else type(activations)())
            if dropout:
...
class EntityModel():
    ...
    def move_model_to_cuda(self):
        if not torch.cuda.is_available():
            logger.info('No CUDA found, using CPU.')
            self._model_device = 'cpu'
            return
